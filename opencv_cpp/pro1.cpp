...
void runProject() {
    ...
    cv::Mat sprite = cv::imread("kimkim.jpg");
    if (sprite.empty()) { std::cerr << "kimkim.jpg 로드 실패\n"; return; }
    ...
    while (true) {
        ...
        //영상의 차이를 줘서 움직임 감지 하기 위해서 두영상을 빼줌 
        ...
        if (score > 15) {
            cv::cvtColor(spriteDraw, spriteDraw, cv::COLOR_BGR2GRAY); //회색 1채널 
            cv::cvtColor(spriteDraw, spriteDraw, cv::COLOR_GRAY2BGR); //회색 3채널 
        }
        ...
        if (!touching && clipped.area() > 0 && ratio > 0.35) {
            ...
            if (!played30 && score >= 30) {
                ...
                //PlaySound소리 제공 함수 ,TEXT 현재 디렉토리 ,NULL 파일재생시 ,SND_ASYNC 파일에서 재생하겠다
            }   //SND_LOOP는 보통 SND_ASYNC랑 같이 써야 의미가 있어. SND_ASYNC: 프로그램 멈추지 않고 소리 재생  SND_LOOP : 그 소리를 계속 반복
