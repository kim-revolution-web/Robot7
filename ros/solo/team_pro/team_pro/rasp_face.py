...
class Controller(Node):
    ...
    def tail_stop(self):
        self.tail_up.angle = -90
        self.tail_down.angle = -90

    def tail_basic(self):
        for _ in range(8):
            for ears_angle in range(-20, 31, 5):
                self.ears.angle = ears_angle

                sleep(0.05)

            for ears_angle in range(30, -21, -5):
                self.ears.angle = ears_angle
                sleep(0.05)


            for basic_angle in range(-90, -40, 5):
                self.tail_up.angle = basic_angle
                self.tail_down.angle = basic_angle
                sleep(0.05)

            for basic_angle in range(-40, -90, -5):
                self.tail_up.angle = basic_angle
                self.tail_down.angle = basic_angle
                sleep(0.05)
    ...
    def tail_angry(self):
        ...
        for _ in range(8):
            ...
            self.tail_down.angle = -70
            sleep(0.15)

            self.tail_up.angle = -50
            self.tail_down.angle = -50
            sleep(0.15)
