...
class Move_turtle(Node):
  def __init__(self):
    ...
    self.ui_sub = self.create_subscription(MyMsg, 'ui_pub_sub',self.ui_cmd,qos,callback_group=self.callback_group)
    # self.ui_service = self.create_service(
    #         String,
    #         'ui_service',
    #         self.ui_service_sever,
  ...
  def ui_cmd(self, msg: MyMsg):
        # msg.data 예: "0.4 -0.2"  (linear angular)
        try:
            # parts = msg.data.strip().split()
            self.linear = float(msg.linear)
            self.angular = float(msg.angular)
             # ✅ 즉시 1번 쏴서 반응 지연 제거
            # self.move_cmd_pub()
  ...
  # def ui_service_sever(self,request,response):
