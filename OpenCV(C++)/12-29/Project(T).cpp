...
void runProject() CV_NOEXCEPT
{
	...
	while (true)
	{
		...
		cv::GaussianBlur(gray_frame, gray_frame, 
			cv::Size(15, 15), 0);
		if (prev_gray.empty()) {
		...
		// absdiff() 명암과 객체 분리
		cv::absdiff(prev_gray, gray_frame, diff); // 차이객체를 diff
		...
		cv::circle(frame, redBall.position,
			redBall.radius, cv::Scalar(0, 0, 255), -1);
